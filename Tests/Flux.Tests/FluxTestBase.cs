...
namespace Flux.Tests
{
    public abstract class FluxTestBase : AbpIntegratedTestBase<FluxTestModule>
    {
        ...
        protected void UsingDbContext(Action<FluxDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<FluxDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FluxDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FluxDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FluxDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FluxDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<FluxDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FluxDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FluxDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FluxDbContext, Task<T>> func)
        {
            T result;
